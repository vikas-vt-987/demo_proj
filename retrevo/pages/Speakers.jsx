          ...
          <div className="flex flex-wrap m-4 ">
             ...
             <a className="block relative  rounded overflow-hidden ">
                ...
                <div className="mt-4 text-center md:text-left">
                  <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Speaker</h3>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:'Speaker'})
  ...
  for (let item of products) {
    if (item.title in Speakers) {
      ...
      if (!Speakers[item.title].size.includes(item.size) && item.availability > 0) {
        Speakers[item.title].size.push(item.size)
      }
    }
  ...
  return {
    ...
    props: {products: JSON.parse(JSON.stringify(Speakers))}, // will be passed to the page component as props
  }
}
