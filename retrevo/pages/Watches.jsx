          ...
          <div className="flex flex-wrap m-4 ">
             ...
             <a className="block relative  rounded overflow-hidden ">
                ...
                <div className="mt-4 text-center md:text-left">
                  <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Smart Watch</h3>
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in Watches) {
      if (!Watches[item.title].color.includes(item.color) && item.availability > 0) {
        Watches[item.title].color.push(item.color)
      }
      if (!Watches[item.title].size.includes(item.size) && item.availability > 0) {
