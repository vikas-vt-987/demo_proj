              ...
              return <Link passHref={true} key={products[item]._id} href={`/product/${products[item].slug} `} >
                ...
                <img alt="ecommerce" className=" items-center m-auto md:m-0 h-[30vh] md:h-[30vh] block justify-center" src={products[item].img} />
                  ...
                  <div className="mt-4 text-center md:text-left">
                    <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Bluetooth Headphone</h3>
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in Headphones) {
      if (!Headphones[item.title].color.includes(item.color) && item.availability > 0) {
        Headphones[item.title].color.push(item.color)
      }
      if (!Headphones[item.title].size.includes(item.size) && item.availability > 0) {
        Headphones[item.title].size.push(item.size)
      }
    }

    else {
      Headphones[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availability > 0) {
        Headphones[item.title].color = [item.color]
        Headphones[item.title].size = [item.size]
      }
    }
  }
  ...
  return {
    ...
    props: { products: JSON.parse(JSON.stringify(Headphones)) }, // will be passed to the page component as props
